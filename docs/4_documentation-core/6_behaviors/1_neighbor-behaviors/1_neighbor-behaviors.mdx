...
Neighbor behaviors are implementations of `INeighborBehaviorJobWrapper`. The `BaseSystem` schedules them after their corresponding neighbor query is done, passing in its results. There are three interfaces relevant in creating a neighbor behavior. The most important one is a custom job `INeighborBehaviorJob<C1, C2, A, R>` which significantly simplifies creating the behaviors. 
