...
- [`[OutData]`](/docs/documentation-core/attributes#outdataattribute) - *declares the type of **results** the combiner accepts `VelocityResults`*
...
If a combiner is declared as accepting `VelocityResults` and a behavior as outputting `VelocityResults`, then they are compatible. When a combiner is selected in the [editor](/docs/documentation-core/editor), only compatible behaviors will be shown as possibilities. This prevents issues when casting from the `IDelayedDisposable`. As usual, the [editor](/docs/documentation-core/editor) will also add whichever [component](/docs/documentation-core/components/intro) is declared in the [`[JobWrapper]`](/docs/documentation-core/attributes#jobwrapperattribute) as the entity's dependency. 
