...
It is however entirely possible to use your own types instead of `VelocityResult` and `DesiredVelocity`. The implementation should declare the types it works using attributes. For example this is the declaration of `CombineVelocitiesJobWrapper`:
...
This job wrapper uses two attributes. First declares what type of objects the combiner accepts. Second declares which components are expected on the entity. The types from the attributes are used in the editor of the `SteeringSystemAsset`. 

- `OutDataAttribute` - *declares what type of results the combiner accepts `VelocityResults`*
- `JobWrapperAttribute` - *declares which components should be present on the entity, above `DesiredVelocityComponent`*
