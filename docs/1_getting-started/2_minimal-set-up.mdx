...
3. Add a `Movement2DAuthoring` [link] component for a `Movement2DSystem` [link].
...
8. Select the *Simple Behaviors* tab and add `GoForwardJobWrapper` from the dropdown. This behavior makes entities go in their forward direction.
...
## Scene Setup
1. Unity's ECS requires working with subscenes [link] for now. Right click into the scene hierarchy, select `NewSubscene` and save it.
...
4. With parameters of `Movement2DAuthoring` [link], you can change their maximum speed, acceleration etc.
5. Hit play
