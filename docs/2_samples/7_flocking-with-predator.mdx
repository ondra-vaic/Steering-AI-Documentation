---
sidebar_position: 8
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# 8. Flocking with Predator

<video controls width="800">
  <source src="/videos/samples/flocking-predator/Video.mp4" type="video/mp4" />
  Your browser does not support the video tag.
</video>

{Description1}

![Minimal Setup folder](/img/samples/flocking-predator/Folder.png)

<Tabs>
  <TabItem value="first" label="Scene"> 

    {Description2}

    <img src="/img/samples/flocking-predator/Scene.png" alt="Description of the image"/>
    
  </TabItem>
  
  <TabItem value="second" label="Steering System Asset"> 

    **Movement2DSteeringAsset**:
    - ***Main Tag*** - `_2_SampleTagComponent`
    - ***Simple Behaviors*** - [`FollowPathJobWrapper`]
    - ***Prefab*** - *Movement2DEntity*

    <img src="/img/samples/flocking-predator/Asset.png" alt="Description of the image"/>

  </TabItem>
  
  <TabItem value="third" label="Prefab"> 
    
    **Behavior Components**:
    - `FollowPathAuthoring`

    **Other Components**:
    - `SteeringEntityAuthoring`
    - `DesiredVelocityAuthoring`
    - `Movement2DAuthoring`
    
    <img src="/img/samples/flocking-predator/Prefab.png" alt="Description of the image"/>

  </TabItem>
</Tabs>